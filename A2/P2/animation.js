...
let animation = {
  setup(p) {
    p.fill(239, 247, 0);
    num_slider = p.createSlider(1, 2000, 500, 20);
    num_slider.position(10, 10);
    d_speed = p.createSlider(0, 10, 1, 0.1);
    d_speed.position(10, 30);
    d_size = p.createSlider(1, 25, 2, 0.0001);
  ...
  draw(p) {
    ...
    p.text(
      `Dot Speed:${Math.round(1000 * 2 ** d_speed.value()) / 1000}`,
      160,
      40
    );
