class dot {
  constructor(p, x, y, c, v_x, v_y, l, dist, speed, resp, view) {
    ...
    this.v_x = v_x || (Math.random() - 0.5) * 2;
    this.v_y = v_y || (Math.random() - 0.5) * 2;
    this.l = l || 1;
    this.dist = dist || 15;
    this.resp = resp || 0;
    this.min =
      Math.abs(255 - this.c[0]) ** 2 +
      Math.abs(255 - this.c[1]) ** 2 +
      Math.abs(255 - this.c[2]) ** 2;
  }
  ...
  draw_eyes(p) {
    ...
    for (
      let min_ang = ang - (Math.PI * this.view) / 4;
      min_ang <= ang + (Math.PI * this.view) / 4;
      min_ang += 0.01 * mul
    ) {
      ...
      off =
        (Math.round(this.y + y_ * dist) * p.width * pd +
          Math.round(this.x + x_ * dist)) *
        4;
      // console.log(off)
